...
# --- CSS: SOLO STILE SENZA BACKGROUND ---
st.markdown("""
<style>
...
# --- DATABASE INTEGRALI (Nature, Item, Abilità) ---
...
def load_assets():
    ...
    try:
        ...
        # CARICAMENTO POKEMON (FIX STR/DICT)
        with open(os.path.join(current_dir, "data", "pokemon_clean.json"), "r", encoding="utf-8") as f:
            ...
            for key, val in items:
                if isinstance(val, dict):
                    ...
                    nm = str(val) if isinstance(data, list) else str(key)
                    tr = "N/A"
    ...
    if not pk_pool: pk_pool = [{'name': 'Pikachu', 'tier': 'N/A'}]
...
with col_main:
    ...
    if st.session_state.step == "DRAFT_PKMN":
        ...
        if len(available) < 3: available = pkmn_pool

    ...
    if st.session_state.step == "DRAFT_PKMN":
        ...
        if not st.session_state.options:
            ...
            if any(p['luck'] for p in st.session_state.options):
                st.toast("🍀 Qualcuno ha sculato...", icon="🎰")
                ...
                if p['luck'] == "MEGA":
                    st.markdown("""
                        <div style="height: 25px; margin-bottom: 5px; display: flex; justify-content: center;">
                            <span style="background: linear-gradient(45deg, #FFD700, #FF8C00); padding: 3px 10px; border-radius: 10px; color: black; font-weight: 900; font-size: 0.75em; box-shadow: 0 0 10px rgba(255,215,0,0.5);">✨ MEGA MUTAZIONE ✨</span>
                        </div>
                    """, unsafe_allow_html=True)

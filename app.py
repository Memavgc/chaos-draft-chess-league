...
# --- NERF PATCH E DATABASE ---
MEGA_MAP = {
    "Venusaur": ["Venusaur-Mega"], "Charizard": ["Charizard-Mega-X", "Charizard-Mega-Y"], "Blastoise": ["Blastoise-Mega"],
    "Alakazam": ["Alakazam-Mega"], "Gengar": ["Gengar-Mega"], "Kangaskhan": ["Kangaskhan-Mega"], "Pinsir": ["Pinsir-Mega"],
    "Gyarados": ["Gyarados-Mega"], "Aerodactyl": ["Aerodactyl-Mega"], "Mewtwo": ["Mewtwo-Mega-X", "Mewtwo-Mega-Y"],
    "Ampharos": ["Ampharos-Mega"], "Scizor": ["Scizor-Mega"], "Heracross": ["Heracross-Mega"], "Houndoom": ["Houndoom-Mega"],
    "Tyranitar": ["Tyranitar-Mega"], "Sceptile": ["Sceptile-Mega"], "Blaziken": ["Blaziken-Mega"], "Swampert": ["Swampert-Mega"],
    "Gardevoir": ["Gardevoir-Mega"], "Sableye": ["Sableye-Mega"], "Mawile": ["Mawile-Mega"], "Aggron": ["Aggron-Mega"],
    "Medicham": ["Medicham-Mega"], "Manectric": ["Manectric-Mega"], "Sharpedo": ["Sharpedo-Mega"], "Camerupt": ["Camerupt-Mega"],
    "Altaria": ["Altaria-Mega"], "Banette": ["Banette-Mega"], "Absol": ["Absol-Mega"], "Glalie": ["Glalie-Mega"],
    "Salamence": ["Salamence-Mega"], "Metagross": ["Metagross-Mega"], "Latias": ["Latias-Mega"], "Latios": ["Latios-Mega"],
    "Rayquaza": ["Rayquaza-Mega"], "Lopunny": ["Lopunny-Mega"], "Garchomp": ["Garchomp-Mega"], "Lucario": ["Lucario-Mega"],
    "Abomasnow": ["Abomasnow-Mega"], "Gallade": ["Gallade-Mega"], "Audino": ["Audino-Mega"], "Diancie": ["Diancie-Mega"],
    "Beedrill": ["Beedrill-Mega"], "Pidgeot": ["Pidgeot-Mega"], "Slowbro": ["Slowbro-Mega"], "Steelix": ["Steelix-Mega"]
}

SPECIAL_TIER = [
    "Articuno", "Zapdos", "Moltres", "Raikou", "Entei", "Suicune", "Lugia", "Ho-Oh", "Regi", 
    "Latias", "Latios", "Kyogre", "Groudon", "Rayquaza", "Deoxys", "Uxie", "Mesprit", "Azelf", 
    "Dialga", "Palkia", "Heatran", "Giratina", "Cresselia", "Phione", "Manaphy", "Darkrai", 
    ...
    "Stakataka", "Blacephalon", "Zeraora", "Meltan", "Melmetal", "Zacian", "Zamazenta", "Eternatus", 
    "Kubfu", "Urshifu", "Zarude", "Glastrier", "Spectrier", "Calyrex", "Great Tusk", "Scream Tail", 
    "Brute Bonnet", "Flutter Mane", "Slither Wing", "Sandy Shocks", "Iron ", "Roaring Moon", 
    "Walking Wake", "Gouging Fire", "Raging Bolt", "Okidogi", "Munkidori", "Fezandipiti", 
    "Ogerpon", "Terapagos", "Pecharunt","Therian","Enamorus"
]
...
def generate_nerfed_draft_options(full_pool, used_groups, num_options=3):
    opts = []
    temp_groups = set(used_groups)
    
    base_pool = [p for p in full_pool if "-Mega" not in p['name'] and "-Primal" not in p['name']]
    random.shuffle(base_pool)
    
    for p in base_pool:
        base_name = p['name']
        grp = get_species_group(base_name)
        
        if grp in temp_groups:
            continue
            
        luck_flag = None 
            
        is_special = any(spec in base_name for spec in SPECIAL_TIER)
        if is_special:
        ...
        if base_name in MEGA_MAP:
            if random.random() <= 0.30:
                final_name = random.choice(MEGA_MAP[base_name])
                luck_flag = "MEGA" 
                
        opts.append({'name': final_name, 'luck': luck_flag})
        temp_groups.add(grp)
...
def load_assets():
    pk_pool = []; mv_db = {}
    pk_banned = ["ZACIAN", "ZAMAZENTA", "ETERNATUS", "MEWTWO", "RAYQUAZA", "KYOGRE", "GROUDON", "ARCEUS", "DIALGA", "PALKIA", "GIRATINA", "XERNEAS", "YVELTAL", "SOLGALEO", "LUNALA", "CALYREX", "KORAIDON", "MIRAIDON", "DEOXYS", "DARKRAI", "TERAPAGOS", "MAGEARNA", "CHI-YU", "CHIEN-PAO", "TING-LU", "WO-CHIEN","CELEBI","DARKRAI-MEGA", "HEATRAN-MEGA", "MAGEARNA-MEGA", "FLOETTE-MEGA","FLOETTE-ETERNAL"]
    
    mv_trash = ["happyhour", "celebrate", "growl", "tailwhip", "leer", "splash", "confide", "camouflage", "bide", "refresh", "soak", "mudsport", "watersport", "snore", "present", "struggle","darkvoid","doubleshock","hyperspacefury","hyperspacehole","toxicthread","tackle","smog","aurawheel","burnup","belch","stuffcheeks","mindreader","poisongas","miracleeye","holdhands","mist","powder","bestow","spitup","attract","captivate","zippyzap", "splishysplash", "floatyfall", "pikapapow", "bouncybubble", "buzzybuzz", "sizzlyslide", "glitzyglow", "baddybad", "sappyseed", "freezyfrost", "sparklyswirl", "veeveevolley"]
...
with col_main:
    # 1. DRAFT POKEMON (CON HUD "SCULATA")
    if st.session_state.step == "DRAFT_PKMN":
        used_groups = {get_species_group(name) for name in st.session_state.used_pkmn}
        
        if not st.session_state.options:
            st.session_state.options = generate_nerfed_draft_options(pk_pool, used_groups, 3)
            
            if any(p['luck'] for p in st.session_state.options):
                st.toast("🍀  hai sculato...", icon="🎰")
        ...
        for i, p in enumerate(st.session_state.options):
            with cols[i]:
                if p['luck'] == "MEGA":
                    st.markdown("""
                        <div style="height: 25px; margin-bottom: 5px; display: flex; justify-content: center;">
                            <span style="background: linear-gradient(45deg, #FFD700, #FF8C00); padding: 3px 10px; border-radius: 10px; color: black; font-weight: 900; font-size: 0.75em; box-shadow: 0 0 10px rgba(255,215,0,0.5);">✨ hai sculato ✨</span>
                        </div>
                    """, unsafe_allow_html=True)
                elif p['luck'] == "LEGENDARY":
                    st.markdown("""
                        <div style="height: 25px; margin-bottom: 5px; display: flex; justify-content: center;">
                            <span style="background: linear-gradient(45deg, #a78bfa, #6d28d9); padding: 3px 10px; border-radius: 10px; color: white; font-weight: 900; font-size: 0.75em; box-shadow: 0 0 10px rgba(167,139,250,0.5);">🎰 DROP RARO 🎰</span>
                        </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown('<div style="height: 25px; margin-bottom: 5px;"></div>', unsafe_allow_html=True)

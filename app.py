...
def display_sprite(name):
    n_raw = name.lower().replace("'", "").replace(" ", "")
    n_clean = n_raw.replace("-", "") 
    base_name = n_raw.split("-")[0].replace("mega", "") 
    
    # Fallback espanso per includere gli sprite Custom/Speculativi di Showdown (es. Mega Z-A)
    urls = [
        f"https://play.pokemonshowdown.com/sprites/ani/{n_clean}.gif",       # 1. Animato pulito
        f"https://play.pokemonshowdown.com/sprites/ani/{n_raw}.gif",         # 2. Animato con trattino
        f"https://play.pokemonshowdown.com/sprites/custom/{n_clean}.png",    # 3. Custom/Speculativo Showdown (Mega Z-A, CAP)
        f"https://play.pokemonshowdown.com/sprites/gen5/{n_clean}.png",      # 4. Statico Gen 9
        f"https://play.pokemonshowdown.com/sprites/ani/{base_name}.gif",     # 5. Fallback Animato Forma Base
        f"https://play.pokemonshowdown.com/sprites/gen5/{base_name}.png"     # 6. Fallback Statico Forma Base
    ]
    
    js_onerror = (
        f"this.onerror=null; this.src='{urls[1]}'; "
        f"this.onerror=function(){{this.onerror=null; this.src='{urls[2]}'; "
        f"this.onerror=function(){{this.onerror=null; this.src='{urls[3]}'; "
        f"this.onerror=function(){{this.onerror=null; this.src='{urls[4]}'; "
        f"this.onerror=function(){{this.onerror=null; this.src='{urls[5]}';}};}};}};}};"
    )
    
    st.markdown(f"""
        <div style="display: flex; justify-content: center; align-items: center; width: 100%; height: 160px; margin-bottom: 10px;">
            <img src="{urls[0]}" onerror="{js_onerror}" style="max-height: 150px; max-width: 100%; object-fit: contain;">
        </div>
    """, unsafe_allow_html=True)

...
def load_assets():
    try:
        ...
        return clean_pkmn, clean_moves, pk_list
    except: return [], {}, []

pkmn_pool, moves_db, raw_pk_list = load_assets()
...
with col_main:
    ...
    if st.session_state.step == "DRAFT_PKMN":
        # LOGICA DI SICUREZZA PER EVITARE VALUEERROR
        available = [p for p in pkmn_pool if p['name'] not in st.session_state.used_pkmn]
        if len(available) < 3:
            available = [p for p in pkmn_pool] # Reset se finiscono
        
        if not st.session_state.options:
            # Controllo finale: se available è ancora vuoto per errori nel JSON
            population = available if len(available) >= 3 else pkmn_pool
            st.session_state.options = random.sample(population, 3)

...
# CSS: Colori dei tipi ripristinati e UI migliorata
st.markdown("""
<style>
...
/* Badge dei Tipi: Colori pieni e testo leggibile */
.type-badge {
...
ITEMS_DB = [
    ...
    "Punching Glove", "Big Root", "Binding Band", "Clear Amulet", "Mirror Herb", "Shell Bell", 
    "Wise Glasses", "Muscle Band", "Leppa Berry", "Custap Berry", "Safety Goggles"
]
...
ABILITIES_DB = [
    ...
    "Primordial Sea", "Delta Stream", "Shadow Tag", "Parental Bond", "Stamina", "Gale Wings"
]

# --- FUNZIONI ---
def get_sprite(name):
    clean_name = "".join(e for e in name.lower() if e.isalnum())
    # URL animato principale
...
def load_assets():
    try:
        ...
        for nm, m in mv_raw.items():
            ...
            clean_moves[nm] = {"type": m.get("type", "Normal"), "bp": m.get("basePower", 0), "cat": cat.replace("Physical", "Fisico").replace("Special", "Speciale").replace("Status", "Stato")}
...
with col_main:
    ...
    if st.session_state.step == "DRAFT_PKMN":
        ...
        for m in st.session_state.options:
            info = moves_db[m]
            st.markdown(f"**{m}** <span class='type-badge' style='background:{TYPE_COLORS.get(info['type'], '#666')}'>{info['type']}</span> <span class='cat-badge'>{info['cat']}</span>", unsafe_allow_html=True)
            if st.button(f"Scegli {m}", key=f"mv_{m}"):
                p['moves'].append(m); st.session_state.options = []; st.session_state.used_moves.add(m)
        ...
        for s in st.session_state.options:
            if st.button(s):
                p['spread'] = s; st.session_state.options = []

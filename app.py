...
# --- CSS: DARK MODE PULITA (Senza Background Meme) ---
st.markdown("""
<style>
...
.move-card {
    ...
    background: rgba(255,255,255,0.05); border-left: 3px solid #f1c40f;
}
</style>
...
def get_sprite(name):
    # Fix per varianti: Articuno-Galar -> articunogalar
...
def load_assets():
    pk_pool = []; mv_pool = {}
    pk_banned = ["ZACIAN", "ZAMAZENTA", "ETERNATUS", "MEWTWO", "RAYQUAZA", "KYOGRE", "GROUDON", "ARCEUS", "DIALGA", "PALKIA", "GIRATINA", "XERNEAS", "YVELTAL", "SOLGALEO", "LUNALA", "CALYREX", "KORAIDON", "MIRAIDON", "DEOXYS", "DARKRAI", "LUGIA", "HO-OH"]
    mv_banned_names = ["darkvoid", "revivalblessing", "sheercold", "guillotine", "horndrill", "fissure", "doubleteam", "minimize", "foresight", "odorsleuth", "miracleeye", "shedtail", "lastrespects"]
    ...
    try:
        ...
        with open(os.path.join(current_dir, "data", "pokemon_clean.json"), "r", encoding="utf-8") as f:
            ...
            for key, val in items:
                nm = val.get('name') if isinstance(val, dict) else (str(val) if isinstance(data, list) else str(key))
                tr = str(val.get('tier', "")).upper() if isinstance(val, dict) else "N/A"
                if nm and "GMAX" not in nm.upper() and tr not in ["UBER", "AG"] and not any(b in nm.upper() for b in pk_banned):
        ...
        with open(os.path.join(current_dir, "data", "moves.json"), "r", encoding="utf-8") as f:
            ...
            for nm, m in mv_data.items():
                ...
                if nm.lower().replace(" ", "").replace("-", "") in mv_banned_names: continue
                if isinstance(m, dict):
                    cat = m.get("category") or m.get("Category") or "Status"
                    mv_pool[nm] = {"type": m.get("type", "Normal"), "bp": m.get("basePower", 0), "cat": cat.replace("Physical", "Fisico").replace("Special", "Speciale").replace("Status", "Stato")}
...
ITEMS_DB = ["Life Orb", "Choice Band", "Choice Specs", "Choice Scarf", "Assault Vest", "Focus Sash", "Leftovers", "Heavy-Duty Boots", "Rocky Helmet", "Expert Belt", "Eviolite", "Air Balloon", "Lum Berry", "Sitrus Berry", "White Herb", "Mental Herb", "Power Herb", "Flame Orb", "Toxic Orb", "Eject Button", "Eject Pack", "Red Card", "Blunder Policy", "Weakness Policy", "Light Clay", "Terrain Extender", "Damp Rock", "Heat Rock", "Smooth Rock", "Icy Rock", "Black Sludge", "Metronome", "Scope Lens", "Razor Claw", "Muscle Band", "Wise Glasses", "Covert Cloak", "Loaded Dice", "Punching Glove", "Clear Amulet", "Mirror Herb", "Shell Bell"]
ABILITIES_DB = ["Huge Power", "Intrepid Sword", "Dauntless Shield", "Mold Breaker", "Intimidate", "Regenerator", "Levitate", "Technician", "Moxie", "Speed Boost", "Contrary", "Adaptability", "Sheer Force", "Sharpness", "Supreme Overlord", "Good as Gold", "Unaware", "Poison Heal", "Guts", "Magic Bounce", "Drizzle", "Drought", "Sand Stream", "Snow Warning", "Libero", "Defiant", "Serene Grace", "Prankster", "Triage", "Beast Boost", "Soul-Heart", "Armor Tail", "Purifying Salt", "Well-Baked Flaw", "Thermal Exchange", "Wonder Guard", "Ice Scales", "Fur Coat", "Simple", "Toxic Debris", "Hadron Engine", "Orichalcum Pulse"]
...
with col_main:
    ...
    # 6. SPREAD (EV CASUALI)
        ...
        st.header("Scegli lo Spread EV (Chaos)")
    ...
    # 7. CONCLUSIONE ED ESPORTAZIONE
        ...
        st.success("Draft Completato! Ecco il tuo team:")
        res = ""

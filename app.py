...
def load_assets():
    ...
    # BANLIST POKEMON (UBER/AG)
    pk_banned = ["ZACIAN", "ZAMAZENTA", "ETERNATUS", "MEWTWO", "RAYQUAZA", "KYOGRE", "GROUDON", "ARCEUS", "DIALGA", "PALKIA", "GIRATINA", "XERNEAS", "YVELTAL", "SOLGALEO", "LUNALA", "CALYREX", "KORAIDON", "MIRAIDON", "DEOXYS", "DARKRAI", "HO-OH", "LUGIA", "RESHIRAM", "ZEKROM", "KYUREM-WHITE"]
    
    # BANLIST MOSSE (OHKO, EVASIONE, BROKEN)
    mv_banned = [
        "darkvoid", "revivalblessing", "sheercold", "guillotine", "horndrill", "fissure", 
        "doubleteam", "minimize", "shedtail", "lastrespects", "ragefist", "populationbomb"
    ]
    ...
    try:
        ...
        with open(os.path.join(current_dir, "data", "pokemon_clean.json"), "r", encoding="utf-8") as f:
            ...
            for key, val in items:
                if isinstance(val, dict):
                    nm = val.get('name') or val.get('Name') or str(key)
                    tr = str(val.get('tier') or "").upper()
                else:
                    nm = str(val); tr = "N/A"
                
                is_banned = any(b in nm.upper() for b in pk_banned)
                if nm and "GMAX" not in nm.upper() and tr not in ["UBER", "AG"] and not is_banned:
        ...
        with open(os.path.join(current_dir, "data", "moves.json"), "r", encoding="utf-8") as f:
            ...
            for nm, m in mv_data.items():
                clean_m_name = nm.lower().replace(" ", "").replace("-", "")
                if clean_m_name in mv_banned:
                    continue
...
# --- RESTO DELLA LOGICA DRAFT (Session State e UI) ---
...
with col_main:
    ...
    if st.session_state.step == "DRAFT_PKMN":
        ...
        if not st.session_state.options:
            st.session_state.options = random.sample(available, min(len(available), 3))
        ...
        if not st.session_state.options:
            m_list = [m for m in moves_db.keys()]
            st.session_state.options = random.sample(m_list, 4)
        for m in st.session_state.options:
            info = moves_db.get(m, {"type": "Normal", "bp": 0, "cat": "Stato"})
            st.markdown(f"**{m}** <span class='type-badge' style='background:{TYPE_COLORS.get(info['type'], '#666')}'>{info['type']}</span>", unsafe_allow_html=True)
    ...
    # [Le fasi ABILITY, ITEM, NATURE, SPREAD rimangono invariate...]
    # (Inserisci qui le fasi mancanti dal codice precedente per completare l'app)

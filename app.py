...
# CSS: Sfondo scuro standard, UI pulita e badges per i tipi
st.markdown("""
<style>
...
# Blacklist potenziata (Inutili + Early Game) - Solar Beam/Blade RIMOSSE (sono ora legali)
BLACKLIST_MOSSE = [
...
def load_assets():
    try:
        ...
        with open(os.path.join(current_dir, "data", "pokemon_clean.json"), "r", encoding="utf-8") as f: pkmn_data = json.load(f)
        with open(os.path.join(current_dir, "data", "moves.json"), "r", encoding="utf-8") as f: moves_data = json.load(f)
        
        # Filtro Pokémon (No Uber, No Gmax)
...
with col_side:
    ...
    st.subheader("🏟️ IL TUO ROSTER")
    if not st.session_state.team: st.info("Drafta i tuoi 6 Pokémon")
...
with col_main:
    ...
    if st.session_state.step == "DRAFT_PKMN":
        ...
        if not st.session_state.options:
            ...
            st.session_state.options = random.sample(available, 3)
    ...
    # 2. DRAFT MOSSE (CON GARANZIA DANNO)
        ...
        if not st.session_state.options:
            ...
            # Garantiamo almeno una mossa di danno
            m_dmg = random.choice(pool_atk)
            others = random.sample([m for m in avail if m != m_dmg], 3)
        ...
        for m in st.session_state.options:
            m_info = moves_db[m]
            with st.container():
                st.markdown(f"**{m}** | {get_type_badge(m_info['type'])} | BP: {m_info['bp']} | {m_info['cat']}", unsafe_allow_html=True)
                if st.button(f"Seleziona {m}", key=f"m_{m}"):
                    p['moves'].append(m)
    ...
    # 3. ABILITÀ
        ...
        st.header(f"Abilità per {p['name'].upper()}")
    ...
    # 4. ITEM
        ...
        p = st.session_state.team[st.session_state.current_pkmn_idx]
        st.header(f"Strumento per {p['name'].upper()}")
    ...
    # 5. NATURE
        ...
        p = st.session_state.team[st.session_state.current_pkmn_idx]
        st.header(f"Natura per {p['name'].upper()}")
    ...
    # 6. SPREAD (CHAOS)
        ...
        p = st.session_state.team[st.session_state.current_pkmn_idx]
        st.header(f"EV Spread per {p['name'].upper()}")
        ...
        for s in st.session_state.options:
            if st.button(s):
                p['spread'] = s
                st.session_state.options = []; st.session_state.used_moves = set() # Reset mosse per il prossimo pkmn
    ...
    # FINISH: EXPORT SHOWDOWN
    elif st.session_state.step == "FINISHED":
        st.header("🏁 DRAFT COMPLETATO!")
        export_text = ""
        for p in st.session_state.team:
            export_text += f"{p['name']} @ {p['item']}\n"
            export_text += f"Ability: {p['ability']}\n"
            export_text += f"EVs: {p['spread']}\n"
            export_text += f"{p['nature']} Nature\n"
            for m in p['moves']: export_text += f"- {m}\n"
            export_text += "\n"
        
        st.subheader("Copia e incolla su Showdown:")

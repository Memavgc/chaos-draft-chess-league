...
def display_sprite(name):
    n = name.lower()
    clean_all = "".join(filter(str.isalnum, n))
    if "mega" in n:
        base_part = n.replace("mega", "").strip("- ")
        url_attempt = f"https://play.pokemonshowdown.com/sprites/ani/{base_part}-mega.gif"
    else:
        url_attempt = f"https://play.pokemonshowdown.com/sprites/ani/{clean_all}.gif"
    
    base_name = "".join(filter(str.isalnum, n.split("-")[0].replace("mega ", "")))
    url_fallback = f"https://play.pokemonshowdown.com/sprites/ani/{base_name}.gif"
    
    st.markdown(f"""
        <div style="height:150px;display:flex;align-items:center;justify-content:center;">
            <img src="{url_attempt}" onerror="this.onerror=null;this.src='{url_fallback}';" style="max-height:150px;">
        </div>
    """, unsafe_allow_html=True)
...
def load_assets():
    pk_pool = []; mv_db = {}
    # BAN SOLO UBER (I Paradox rimangono nel pool)
    pk_banned = ["ZACIAN", "ZAMAZENTA", "ETERNATUS", "MEWTWO", "RAYQUAZA", "KYOGRE", "GROUDON", "ARCEUS", "DIALGA", "PALKIA", "GIRATINA", "XERNEAS", "YVELTAL", "SOLGALEO", "LUNALA", "CALYREX", "KORAIDON", "MIRAIDON", "DEOXYS", "DARKRAI", "TERAPAGOS", "MAGEARNA"]
    mv_trash = ["happyhour", "celebrate", "growl", "tailwhip", "leer", "splash", "confide", "camouflage", "bide", "refresh", "soak", "mudsport", "watersport", "snore", "present", "struggle", "darkvoid"]
    ...
    try:
        ...
        with open(os.path.join(current_dir, "data", "moves.json"), "r", encoding="utf-8") as f:
            ...
            for nm, m in mv_data.items():
                ...
                if any(x in nm.upper() for x in ["G-MAX", "GMAX", " Z-", "MAX "]) or clean_m in mv_trash: continue
...
with col_side:
    ...
    for i, p in enumerate(st.session_state.team):
        with st.expander(f"**{i+1}. {p['name'].upper()}**", expanded=True):
            ...
            if p['moves']: st.markdown("  \n".join([f"🔹 {m}" for m in p['moves']])) # REINSERITO RECAP MOSSE

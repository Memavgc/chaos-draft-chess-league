...
import urllib.request # <-- NUOVA IMPORTAZIONE FONDAMENTALE
...
# --- NUOVO MOTORE SPRITE: CONTROLLO IN PYTHON ---
...
def get_valid_sprite_url(name):
    ...
    # Lista di tutti i tentativi possibili
    urls_to_try = [
        f"https://play.pokemonshowdown.com/sprites/ani/{n_clean}.gif",        # 1. Mega ufficiali pulite (latiasmega)
        f"https://play.pokemonshowdown.com/sprites/ani/{n_raw}.gif",          # 2. Forme con trattino
        f"https://play.pokemonshowdown.com/sprites/custom/{n_clean}.png",     # 3. Custom/ZA
        f"https://play.pokemonshowdown.com/sprites/gen5/{n_clean}.png",       # 4. Statico Gen 9
        f"https://play.pokemonshowdown.com/sprites/ani/{base_name}.gif",      # 5. Animato Forma Base (latias)
        f"https://play.pokemonshowdown.com/sprites/gen5/{base_name}.png",     # 6. Statico Forma Base
        "https://play.pokemonshowdown.com/sprites/items/poke-doll.png"        # 7. Sostituto
    ]
    
    # Python prova in millisecondi quale URL esiste davvero
...
def display_sprite(name):
    # Ottiene l'unico URL garantito al 100%
    valid_url = get_valid_sprite_url(name)
    
    # Niente più JS o onerror: stampiamo solo l'immagine sicura e perfettamente centrata
    st.markdown(f"""
    ...
    """, unsafe_allow_html=True)# --- DATABASE DIZIONARI CON DESCRIZIONI ---
ABILITIES = {
...
# --- CARICAMENTO ASSET E FILTRI MOSSE DEFINITIVI ---
@st.cache_data(show_spinner=False)
def load_assets():
    pk_pool = []; mv_db = {}
    ...
    try:
        ...
        # 2. Caricamento e Filtro Rigido Mosse
        with open(os.path.join(current_dir, "data", "moves.json"), "r", encoding="utf-8") as f:
            ...
            for nm, m in mv_data.items():
                ...
                # BAN A: Mosse Z e Dynamax
                if any(x in nm.upper() for x in ["G-MAX", "GMAX", " Z-", "MAX ", " Z "]) or m.get("isZ") or m.get("isMax"):
                    continue
                
                # BAN B: Mosse inutili (Status/Danno spazzatura)
                if clean_m in mv_trash:
                    continue
                    
                # BAN C: Mosse deboli (Potenza < 55), ma salviamo Hidden Power
                if 0 < bp < 55 and clean_m != "hiddenpower":
                    continue

                # Se sopravvive ai ban, viene aggiunta al database giocabile

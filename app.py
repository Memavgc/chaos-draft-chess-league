...
@st.cache_data
def load_assets():
    with open("data/pokemon_clean.json", "r", encoding="utf-8") as f:
        pkmn = json.load(f)
    ...
    m_data = {nm: {
        ...
        "cat": m["category"].replace("Physical", "Fisico").replace("Special", "Speciale").replace("Status", "Stato"),
        "desc": m.get("shortDesc", "Nessuna descrizione disponibile.")
    } for nm, m in moves.items() if m.get("num", 0) > 0 and not m.get("isZ")}
...
with col_main:
    ...
    if st.session_state.step == "DRAFT_PKMN":
        ...
        if not st.session_state.options: st.session_state.options = random.sample(pkmn_pool, 3)
        cols = st.columns(3)
        for i, p in enumerate(st.session_state.options):
            with cols[i]:
                name = p['name']; clean = name.lower().replace(" ", "").replace("-", "")
                ...
                if st.button(f"Scegli {name.upper()}", key=f"pk_{i}"):
                    ...
                    st.session_state.options = []; st.session_state.step = "MOVES"; st.rerun()
